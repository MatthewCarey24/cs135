    ...
    * self.b : float
    ...
    def fit(self, x_NF, y_N):
        ...
        * self.b (scalar real bias, if desired)
        ...
        xTwiddle_NF = np.append(x_NF, np.ones((N,1)), 1)

        xTx_FF = xTwiddle_NF.T @ xTwiddle_NF
        xy = xTwiddle_NF.T @ y_N
        theta_F1 = np.linalg.solve(xTx_FF, xy)
